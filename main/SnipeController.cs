...
public class SnipeController : IDisposable
{
            ...
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (!baselineSet)
                    {
                        ...
                        baselineSet = true;
                        Log($"[SNIPE] Baseline: {known.Count} existing instance(s)");
                        continue;
                    }
                    ...
                    foreach (var inst in instances.Cast<JObject>())
                    {
                        ...
                        if (cfg.AccessTypes != null && cfg.AccessTypes.Count > 0 &&
                            !cfg.AccessTypes.Contains(accessType))
                        {
    ...
    private async Task TryJoinAsync(string location, CancellationToken ct)
    {
        ...
        try
        {
            Log($"[SNIPE] Attempting self-invite to {location}");
            ...
            if (ok)
            {
                Log($"[SNIPE] Self-invite succeeded for {location}");
                return;
            }

            Log($"[SNIPE] Self-invite failed — falling back to launch URI");
            ...
            try
            {
                ...
                _core.SendToJS("snipeJoinResult", new { success = true, location, method = "launchUri" });
            }
            catch (Exception ex)
            {
                Log($"[SNIPE] Launch URI failed: {ex.Message}");
